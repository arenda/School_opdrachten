...
function inschrijvingen($conn)
{
    if (isset($_POST['submit'])) {
        ...
        $emailLeider = $_POST['emailLeider'];
        $leden = $_POST['leden'];
        $motivatie = $_POST['motivatie'];
        $opdrachtNaam = $_POST['opdrachtNaam']; // voeg deze regel toe

        if (empty($groepNaam) || empty($emailLeider) || empty($leden) || empty($motivatie) || empty($opdrachtNaam)) {
            echo "All fields are required.";
            return;
        }
        ...
        try {
            ...
            $stmtUpdate = $conn->prepare("INSERT INTO `inschrijvingen` (`groepNaam`,`emailLeider`, `leden`, `motivatie`, `opdrachtNaam`) 
                        VALUES (:groepNaam, :emailLeider, :leden, :motivatie, :opdrachtNaam)");
            $stmtUpdate->bindParam(':groepNaam', $groepNaam);
            $stmtUpdate->bindParam(':emailLeider', $emailLeider);
            $stmtUpdate->bindParam(':leden', $leden);
            $stmtUpdate->bindParam(':motivatie', $motivatie);
